namespace :db do
  ...
  task remake_statistic_data: :environment do
    ...
    Profile.includes(:trainee_type, :language, :location, :stage).each do |profile|
      TraineeType.all.each do |trainee_type|
          ...
          Location.all.each do |location|
            if profile.start_training_date
              statistic = Statistic.find_or_create_by month: profile.start_training_date.to_date.beginning_of_month,
                trainee_type: trainee_type, language: language,
                location: location, stage: @in_education
              if profile.trainee_type == trainee_type && profile.language == language &&
