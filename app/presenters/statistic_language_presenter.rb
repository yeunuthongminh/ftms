class StatisticLanguagePresenter < ActionView::Base
  def initialize trainee_by_language
    @trainee_by_language = trainee_by_language
  end

  def render
    sidebar = Array.new
    body = Array.new
    @trainee_by_language.each_with_index do |items, index|
  ...
  def body_item body_items, index
    "<div class=\"trow list_#{index}\" id=\"body-row-#{index}\">
      <div class=\"tcell total-trainees-language-#{body_items[:name]}
        number_trainees text-right trainee-by-language\"
